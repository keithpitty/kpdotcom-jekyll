...
After a little research, I decided that Arya Asemanfar's "tm_syntax_highlighting plugin":http://github.com/arya/tm_syntax_highlighting/tree/master looked the most promising. Arya's "blog post":http://unboundimagination.com/Rails-Plugin:-TextMate-Syntax-Highlighting about it was particularly helpful.

h3. Prerequisites

Install the oniguruma system library (see link in the plugin README).

Install the "ultraviolet":http://ultraviolet.rubyforge.org/ gem, which will also install the textpow and oniguruma gems.

<code>sudo gem install ultraviolet</code>

h3. 1. Install the Plugin

<code>./script/plugin install git://github.com/arya/tm_syntax_highlighting</code>

h3. 2. Copy all themes from ultraviolet:

<code>./script/generate syntax_css</code>

h3. 3. Create defaults initializer
...
h3. 4. Create an application helper method to parse the Textile and code:
...
h3. 5. Use the helper in the views

<code lang='ruby_on_rails'><%= parse_textile_and_code_syntax(@blog_post.post) %></code>

h3. 6. Add CSS for horizontal scrolling
